...
# Local-only "Ask Claude" chat panel. Activates ONLY when the page is served from
# localhost by harness.ask_server (which backs /ask with `claude -p`, i.e. your
# subscription). On the public site (github.io) it no-ops, leaving the deep-link
# buttons. Self-use only.
SITE_ASK_JS = """\
(function () {
...
def _paper_table_row(e: dict, link_prefix: str) -> str:
    """A 'Date | Paper | Venue | Why selected' row; `link_prefix` points at the
...
def _review_page_lines(conn, cfg) -> list[str]:
    ...
    if body:
        ...
        lines += ["> [!note] No review yet", "",
                  "_The weekly review runs on the first overnight run of each ISO week, "
...
def build_site(conn, cfg: Config) -> None:
    ...
    for topic in cfg.topics:
        ...
        for year in years:
            ...
            for row in rows:
                ...
                is_key = _is_key(row["title"], key_norms)
                if is_key:
                ...
                e = {"date": date, "title_esc": _esc(row["title"]), "pslug": pslug,
                     "venue_cell": venue_cell, "venue_text": v, "has_venue": bool(v),
    ...
    (docs / "javascripts" / "ask.js").write_text(SITE_ASK_JS, encoding="utf-8")
